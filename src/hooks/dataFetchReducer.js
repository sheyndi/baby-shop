...
export const dataFetchReducer = (state, action) => {
    switch (action.type) {
        ...
        case STATUS.ERROR_SYSTEM:
            return { ...state, status: STATUS.ERROR, data: null, error: action.payload };
        case STATUS.IDLE:
...
export const useDataFetchReducer = () => {
    ...
    const setError = (error) => {
        if (error.response && error.response.status >= 400 && error.response.status && error.response.status < 500) {
            dispatch({ type: STATUS.ERROR_USER, payload: error.response.data });
        } else {
            dispatch({ type: STATUS.ERROR_SYSTEM, payload: null });
        }
    }
    /**
...
export function useApiRequestState() {
    ...
    return { requestStatus, responseData, requestError, executeRequest, resetState, updateData };
}